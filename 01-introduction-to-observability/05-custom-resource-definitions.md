...
kubectl run curl-test --rm -it --restart=Never --image=curlimages/curl:8.10.1 -- \
  curl -s http://example-app.default.svc.cluster.local:8080/metrics | grep -v "^#"
```
...
One metric. That is worth understanding, because it explains something that confuses people later.

The application also exposes a counter called `http_requests_total`, but a counter that has never incremented is not emitted at all. Nobody has sent this app a request yet, so the counter does not exist. It will appear the moment the app serves its first request.

This is normal Prometheus behaviour, not a fault. A metric with no observations is absent rather than zero.

### Step 1b: Generate some traffic

Send the app some requests so `http_requests_total` comes into existence:

```bash
kubectl run curl-test --rm -it --restart=Never --image=curlimages/curl:8.10.1 -- \
  sh -c 'for i in $(seq 1 30); do curl -s -o /dev/null http://example-app.default.svc.cluster.local:8080/; done; echo done'
```

Confirm the counter now exists:

```bash
kubectl run curl-test --rm -it --restart=Never --image=curlimages/curl:8.10.1 -- \
  curl -s http://example-app.default.svc.cluster.local:8080/metrics | grep http_requests_total
```

You should now see a line like `http_requests_total{code="200",method="get"} 7`.

Note the number will be smaller than 30. The Service load-balances across both pods, so each one only saw part of the traffic.
...
Data. One series per pod, each carrying `pod`, `namespace`, `service`, `instance`, `code` and `method` labels.

Look at those labels carefully. The application sent none of them except `code` and `method`. Prometheus attached the rest from the Kubernetes metadata it discovered during the scrape. That is the operator's relabelling at work.

Also notice the counts differ between pods, for example 6 on one and 10 on another. The Service load-balanced the traffic unevenly, which is normal. It is a small but real demonstration of why per-pod series matter: a single aggregated number would have hidden that.
...
**Target is UP but the query returns no data**

The scrape is working and the metric simply does not exist yet. A Prometheus counter that has never incremented is not emitted at all, so `http_requests_total` is absent until the application serves its first request.

Confirm by querying `up{job="example-app"}`. If that returns 1, discovery and scraping are fine, and you only need to generate traffic. Check what the target is actually exposing with the curl command from Step 1.

Absent is not the same as zero. This catches people regularly.
...
- A counter that has never incremented is absent, not zero. An empty query result can mean the scrape is fine and the metric has simply never been observed.
